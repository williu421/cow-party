...
def setUpGame(self):
    ...
    self.PieceGroup=pygame.sprite.Group() #use Pygame's Group to update all pieces simultaneously 
    self.PieceGroup.add(self.me)
    self.diceGroup=pygame.sprite.Group()
    self.screenGroup=pygame.sprite.Group() #for screens that stop the flow of the game 
    self.me= Piece("lonely",14,14,True) #last parameter says it's me 
    
    cols,rows=15,15
    self.gameBoard = Board(self.height,self.width,cols,rows,self) #uses the Board class 
    ...
    self.turnHold=False 
    self.displayMessage=False #signals if there needs to be an overlay with a message on it 
...
def moveCheck(self,dt):
    if self.movesLeft == None: #i.e. game just started 
        ...
        return 
    if self.movesLeft == 0: 
        self.piecesDict[self.turnPlayer].turnsDone+=1 #signal that the previous player completed the turn
        nextTurn(self)
    else: 
...
def drawBeansAndCoffee(outerGame,screen,x,y,PID):
    ...
    if outerGame.movesLeft!=None:
        namesText=nameFont.render('movesLeft: %d' \
            %(outerGame.movesLeft),False,(0,0,0))
        screen.blit(namesText,(x+60,y+150))
